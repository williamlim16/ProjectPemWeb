...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    for ($i = 0; $i < count($_POST['name']); $i++) {
        ...
        $user_username = sterilize_input($_POST['name'][$i]); //new addition, database's primary key
        $user_skills = sterilize_input($_POST['skills'][$i]); //new addition, database's primary key
        $user_percent = sterilize_input($_POST['percent'][$i]); //new addition, database's primary key
